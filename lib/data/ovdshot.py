...
COCO_NOVEL_CLASS_NAMES = ['airplane', 'bus', 'cat', 'dog', 'cow', 'elephant', 'umbrella', \
    'tie', 'snowboard', 'skateboard', 'cup', 'knife', 'cake', 'couch', 'keyboard', \
    'sink', 'scissors']

COCO_BASE_CLASS_NAMES = ['person', 'bicycle', 'car', 'motorcycle', 'train', 'truck', \
...
for cls in COCO_NOVEL_CLASS_NAMES:
    for v in  COCO_CATEGORIES:
        if v['isthing'] == 1 and v['name'] == cls:
            COCO_NOVEL_CATEGORIES.append(v)

def register_cd_vito(root=os.environ.get("CDVITO_DATASETS",
                                         "/local_datasets/CD_VITO/cdvito_datasets")):
    specs = {
        "DIOR":   ("DIOR",   "10_shot.json", "test.json", "train", "test"),
        "FISH":   ("FISH",   "10_shot.json", "test.json", "train", "test"),
        "UODD":   ("UODD",   "10_shot.json", "test.json", "train", "test"),
        "NEUDET": ("NEU-DET","10_shot.json", "test.json", "train", "test"),
    }

    def _reg(name, ann, img_root):
        if name in DatasetCatalog.list():
            return
        if not os.path.isfile(ann):
            raise FileNotFoundError(f"[register_cd_vito] '{name}'의 어노테이션 파일이 없습니다: {ann}")
        if not os.path.isdir(img_root):
            raise NotADirectoryError(f"[register_cd_vito] '{name}'의 이미지 폴더가 없습니다: {img_root}")
        register_coco_instances(name, {}, ann, img_root)

    for key, (folder, tr_json, te_json, tr_dir, te_dir) in specs.items():
        ann_dir = os.path.join(root, folder, "annotations")
        # 10-shot train
        _reg(f"{key}_10shot",
             os.path.join(ann_dir, tr_json),
             os.path.join(root, folder, tr_dir))
        # test
        _reg(f"{key}_test",
             os.path.join(ann_dir, te_json),
             os.path.join(root, folder, te_dir))
...
def _get_coco_fewshot_instances_meta():
    ...
    base_categories = [
        ...
        if k["isthing"] == 1 and k["name"] in COCO_BASE_CLASS_NAMES
    ]
...
def load_coco_json(json_file, image_root, metadata, dataset_name):
    ...
    if is_shots:
        fileids = {}
        split_dir = os.path.join(os.environ.get("DETECTRON2_DATASETS", "datasets"), "cocosplit2017/seed1")
        # split_dir = "/scratch/xz653/code/RegionCLIP/datasets/cocosplit2017/seed1"
...
def register_all_coco(root=None):
    ...
    for shot in [5, 10, 30]:
        for prefix in ["novel", "all"]: # "all"
            name = f"fs_coco17_support_{prefix}_{shot}shot"
            METASPLITS.append((name, "coco/train2017", ""))
    ...
    for name, imgdir, annofile in METASPLITS:
        ...
        register_meta_coco(
            ...
            os.path.join(root, annofile) if annofile else None,
        )
...
if __name__ == "__main__":
    dname = "fs_coco17_support_novel_10shot"
    Dnovel = DatasetCatalog.get(dname)
    Mnovel = MetadataCatalog.get(dname)

    dname = "fs_coco17_train_base"
    Dbase = DatasetCatalog.get(dname)
    Mbase = MetadataCatalog.get(dname)
